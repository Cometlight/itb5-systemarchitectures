...
/**
 * This filter expects the bonding discs to be completely white: pixel value of
 * 255 on a scale of 0..255, while all other pixels in the image are expected to have a
 * pixel value < 255.
 */
public class CalcCentroidsFilter extends DataEnrichmentFilter<PlanarImage, LinkedList<Coordinate>> {
		...
		for (int x = 0; x < bi.getWidth(); x++) {
			for (int y = 0; y < bi.getHeight(); y++) {
				...
				if (p == 255 && _general.containsKey(new Coordinate(x, y)) == false) {
					getNextFigure(bi, x, y); // if there is a not visited white
												// pixel, save all pixels
	...
	private void addConnectedComponents(BufferedImage img, LinkedList<Coordinate> figure, int x, int y) {
		if (x - 1 >= 0 && _general.containsKey((new Coordinate(x - 1, y))) == false
		...
		if (x + 1 < img.getWidth() && _general.containsKey((new Coordinate(x + 1, y))) == false
				&& img.getRaster().getSample(x + 1, y, 0) == 255) {
			_general.put(new Coordinate(x + 1, y), true);
			figure.add(new Coordinate(x + 1, y));
			addConnectedComponents(img, figure, x + 1, y);
		}
		if (y - 1 >= 0 && _general.containsKey((new Coordinate(x, y - 1))) == false
		...
		if (y + 1 < img.getHeight() && _general.containsKey((new Coordinate(x, y + 1))) == false
				&& img.getRaster().getSample(x, y + 1, 0) == 255) {
			_general.put(new Coordinate(x, y + 1), true);
			figure.add(new Coordinate(x, y + 1));
			addConnectedComponents(img, figure, x, y + 1);
	...
	private Coordinate[] calculateCentroids() {
		...
		for (LinkedList<Coordinate> figure : _figures) {
			...
			// Some of the following lines had to be commented out,
			// otherwise it wouldn't work.
			centroids[i] = new Coordinate(xMedian,
					// +
					// (int)_image.getProperty("ThresholdX"),
					yMedian// +
					// (int) _image.getProperty("ThresholdY")
			);
			i++;
		}
		return centroids;
